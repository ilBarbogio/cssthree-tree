...
export class Tree {
    ...
    */
    ...
    computeSpacing() {
        ...
        this.nodes.forEach((el) => {
            if (el.type == "standard")
                el.object3D.position.set(width * (el.pos - .5), -height * el.lvl, 0);
            else
                el.object3D.position.set(width * (el.pos - .5), -height * el.lvl, 0);
        });
...
class Node {
    ...
    constructor(id) {
        ...
        if (element.getAttribute("parents"))
            this.parents = element.getAttribute("parents").split(" ").map((el) => { return Number(el); });
        if (element.getAttribute("children"))
            this.children = element.getAttribute("children").split(" ").map((el) => { return Number(el); });
...
class Edge {
    ...
    constructor(parent, child) {
        ...
        if (parent.object3D.position.x > child.object3D.position.x)
            this.angle *= -1;
...
//# sourceMappingURL=cssthree-tree.js.map