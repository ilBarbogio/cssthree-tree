import renderMath from 'katex/dist/contrib/auto-render'
...
export class Albero{
  ...
  constructor(larghezza:number, altezza:number, dove:HTMLElement|string){
    ...
    if(typeof dove == "string") this.contenitore=document.getElementById(dove)
    else this.contenitore=dove
  ...
  onResize() {
    ...
    (this.camera as any).aspect = larghezza / altezza;
  ...
  leggiPagina=function(){
    ...
    nodiHTML.forEach((el:HTMLElement)=>{
  ...
  calcolaSpaziature(){
    ...
    //posizione nodi
    this.nodi.forEach((el)=>{
      if(el.tipo=="normale") el.oggetto3D.position.set(width*(el.pos-.5),-height*el.lvl,0)
      else el.oggetto3D.position.set(width*(el.pos-.5),-height*el.lvl,0)
    })
  ...
  getNodiByLvl(lvl){
    let collezione=[]
    this.nodi.forEach((el)=>{
      if(el.lvl==lvl) collezione.push(el)
    })
    return collezione
  }
  getNodiByPos(pos){
    let collezione=[]
    this.nodi.forEach((el)=>{
      if(el.pos==pos) collezione.push(el)
    })
    return collezione
  }
  ...
  punto(x,y,z){
    ...
    let oggetto=new CSS3DObject(punto);
...
class Nodo{
  ...
  constructor(id){
    ...
    if(elemento.getAttribute("padri")) this.padri=elemento.getAttribute("padri").split(" ").map((el)=>{return Number(el)})
    if(elemento.getAttribute("figli")) this.figli=elemento.getAttribute("figli").split(" ").map((el)=>{return Number(el)})
    if(elemento.getAttribute("colore")) this.colore=elemento.getAttribute("colore")
  ...
  creaCSS3DObject(){
    ...
    this.oggetto3D=new CSS3DObject(wrapper);
...
class Arco{
  ...
  constructor(padre,figlio){
    ...
    if(padre.oggetto3D.position.x>figlio.oggetto3D.position.x) this.angolo*=-1
